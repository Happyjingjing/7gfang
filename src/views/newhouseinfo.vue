<template>
  <div class="newhouseinfo"
       v-wechat-title="this.info.title">
    <img src="@/assets/img/rightbtn.png"
         alt=""
         class="menutopimg"
         @click="menulist"
         v-if="ismenu" />
    <div class="headerhouse"
         v-if="isheaderhouse">
      <div class="navprevbox">
        <img src="@/assets/img/housenavlef.png"
             alt=""
             class="navprve"
             @click="$router.back(-1)" />
      </div>
      <div class="menubox"
           @click="menulist">
        <img src="@/assets/img/housenavrig.png"
             alt=""
             class="menu" />
      </div>
    </div>
    ...
    <div class="housebox">
      <div class="houseinfo">
        <h1>{{ info.title }}</h1>
        <p class="tag">
          <span v-for="(item, index) in likeList"
        ...
        <div class="txtbox">
          <p class="price"
             v-if="info.price">
            均价
            <span class="red">{{ info.price }}</span>元/m²
          </p>
          <p class="price"
             v-else>
            均价
          ...
          <p>
            开盘日期
            <span class="hui"
                  v-if="info.start_date">{{ info.start_date }}
            </span>
            <span class="hui"
                  v-else>暂无数据</span>
          </p>
          <p v-if="tophuxing.length > 0">
            ...
            <span class="hui"
                  ...
                  :key="index">{{ item.apartment_shi | replace }}室{{ item.apartment_ting | replace }}厅{{ item.apartment_wei | replace }}卫（建筑面积{{ item.house_area }}m²）
            </span>
          </p>
          <p v-else>
            主推户型
        ...
        <div class="tixingbox">
          <p class="tip"
             @click="tiptan(0)">
            <i class="timeico"></i>开盘提醒</p>
          <p class="tip">|</p>
          <p class="tip"
             @click="tiptan(1)">
            <i class="priceico"></i>变价通知</p>
        </div>
      </div>
      <div class="telbox">
        <div class="lef">
        ...
        <div class="shapbd">
          <div class="content">
            <img :src="shapanimg"
                 alt="">
            <ul class="lppage">
              <li v-for="(item, index) in pointdata"
                  :key="item.id"
                  :style="{'top':item.y+'px','left':item.x+'px'}">{{item.name}}</li>
            </ul>
          </div>
        </div>
      ...
      <div class="xiangcebox">
        ...
        <div class="picbd">
          <ul class="clearfix"
              ref="picul"
              :style="{ width: widthpic }">
            <li v-for="(item, index) in xiangce"
                :key="index"
      ...
      <div class="dianpingbox">
        ...
        <div class="dianpbd">
          ...
          <div class="topbox">
            <img src="@/assets/img/pinglico.png"
                 alt=""
                 class="pen" />
            <input type="text"
                   placeholder="期待您的点评"
