      ...
      <div class="content">
        ...
        <div class="lppage">
          <span class="swbullet"
                ...
                :key="item.id"
                :style="{'left':item.x + 'px','top':item.y + 'px'}">{{item.name}}</span>
        </div>
...
export default {
  ...
  methods: {
    ...
    lpswfun () {
      ...
      var contentWidthEdge = content.clientWidth - document.documentElement.clientWidth; // 900-375
      ...
      function move (event) {
        ...
        left = isNaN(parseInt(content.style.left)) ? 0 : parseInt(content.style.left);
        left = left + offset.x;
        ...
        ltop = isNaN(parseInt(content.style.top)) ? 0 : parseInt(content.style.top);
        ltop = ltop + offset.y;
